...
//
// This is the identity module's own vocabulary. The ticket module declares an
// ActorRole holding the same three strings, and that duplication is deliberate:
// the role on a users row is what someone *is*, and the role on a status
// history row is what someone *was when they acted*. Promoting an agent must
// not rewrite the audit trail, so the two cannot be the same value even though
// they read alike.
//
// internal/app maps between them, and is the only package allowed to. See
// docs/adr/0005.
type Role string
...
// User is one of our users: a Clerk identity we have a row for.
//
