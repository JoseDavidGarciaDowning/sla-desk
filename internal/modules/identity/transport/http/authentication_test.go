...
// chain builds the production middleware stack by hand: the SDK verifies and
// caches, RequireAuth rejects and resolves.
//
// It is the same pair identity.Module.Authenticate assembles, in the same
// order. That the module really does assemble it — and that every protected
// route is actually mounted behind it — is asserted where the mounting happens,
// in internal/app/router_test.go. Here the point is the verification path
// itself.
func chain(t *testing.T, stub *clerkStub, users application.UserRepository, next http.Handler) http.Handler {
	...
	svc := application.NewService(users, clerk.NewIdentityProvider(cfg))
	return clerk.Middleware(cfg)(identityhttp.RequireAuth(svc)(next))
}
...
func TestIdentityFetcherReadsThePrimaryEmail(t *testing.T) {
	...
	provider := clerk.NewIdentityProvider(clerk.Config{
		SecretKey: "sk_test_not_a_real_key",
		APIURL:    f.stub.URL,
	})

	got, err := provider.Fetch(t.Context(), "user_known")
	if err != nil {
