...
// MePath is where a caller reads back who our database says they are.
const MePath = "/me"
...
// MeHandler answers with the authenticated caller's own id and role.
//
...
func MeHandler() http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		...
		if err := json.NewEncoder(w).Encode(meResponse{ID: user.ID, Role: user.Role}); err != nil {
