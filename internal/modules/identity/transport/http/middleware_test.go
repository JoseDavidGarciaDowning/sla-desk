...
// fakeUsers stands in for the postgres repository. A fake rather than a mock:
// the tests assert on what the middleware produced, not on which methods it
// happened to call — except for the upsert, where "was it called at all" is the
...
func (f *fakeUsers) ByClerkID(context.Context, string) (domain.User, error) {
	if f.getErr != nil {
...
func (f *fakeUsers) Upsert(_ context.Context, clerkUserID string, id domain.Identity) (domain.User, error) {
	f.upserts++
...
// fakeIdentities stands in for the Clerk Backend API. The session token carries
...
func (f *fakeIdentities) Fetch(context.Context, string) (domain.Identity, error) {
	f.calls++
...
func withClaims(subject string) *http.Request {
	...
	claims := &clerksdk.SessionClaims{
		RegisteredClaims: clerksdk.RegisteredClaims{Subject: subject},
	}
	return r.WithContext(clerksdk.ContextWithSessionClaims(r.Context(), claims))
}
...
func TestRequireAuthRejectsARequestWithNoSessionClaims(t *testing.T) {
	...
	handler := requireAuth(&fakeUsers{}, &fakeIdentities{})(protected.handler())
...
func TestRequireAuthPutsOurUserInTheContext(t *testing.T) {
	...
	users := &fakeUsers{user: want}

	var got domain.User
	var ok bool
	handler := requireAuth(users, &fakeIdentities{})(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
...
func TestRequireAuthProvisionsAUserItHasNeverSeen(t *testing.T) {
	...
	users := &fakeUsers{getErr: application.ErrUserNotFound, user: provisioned}
	clerkAPI := &fakeIdentities{identity: domain.Identity{Email: "new@example.test", Name: "New Person"}}
...
func TestRequireAuthDoesNotCallClerkForAKnownUser(t *testing.T) {
	users := &fakeUsers{user: domain.User{ClerkUserID: "user_known", Role: domain.RoleCustomer}}
	clerkAPI := &fakeIdentities{}
...
func TestRoleComesFromOurTableNotFromTheToken(t *testing.T) {
	...
	r := httptest.NewRequest(http.MethodGet, "/api/tickets", nil)
	claims := &clerksdk.SessionClaims{
		RegisteredClaims: clerksdk.RegisteredClaims{Subject: "user_ambitious"},
	}
	// Whatever a client manages to put in the token, including a role.
	claims.Custom = map[string]string{"role": "admin"}
	r = r.WithContext(clerksdk.ContextWithSessionClaims(r.Context(), claims))

	var got domain.User
	handler := requireAuth(users, &fakeIdentities{})(http.HandlerFunc(func(_ http.ResponseWriter, r *http.Request) {
		got, _ = identityhttp.UserFromContext(r.Context())
	}))
...
func TestRequireAuthFailsClosedWhenClerkIsUnreachable(t *testing.T) {
	users := &fakeUsers{getErr: application.ErrUserNotFound}
	clerkAPI := &fakeIdentities{err: errors.New("clerk: connection refused")}
...
func TestRequireAuthFailsClosedWhenTheDatabaseErrors(t *testing.T) {
	// Not ErrUserNotFound: a database that is broken must not be mistaken for a
	// user we have never seen, or every outage turns into a provisioning storm.
	users := &fakeUsers{getErr: errors.New("connection reset by peer")}
	clerkAPI := &fakeIdentities{}
...
// Clerk does not guarantee a name, and an absent one must stay distinguishable
// from a blank one all the way down.
//
// This asserts only that the empty string survives to the repository port. That
// it then lands in the column as NULL rather than as ” is the repository's
// job, and is asserted against a real database in
// internal/modules/identity/infrastructure/postgres — the conversion moved
// there when the module took ownership of the users table.
func TestProvisioningWithoutANamePassesAnEmptyOne(t *testing.T) {
	users := &fakeUsers{getErr: application.ErrUserNotFound}
	clerkAPI := &fakeIdentities{identity: domain.Identity{Email: "noname@example.test"}}
	...
	if users.upsertIdentity.Name != "" {
		t.Errorf("name = %q, want empty", users.upsertIdentity.Name)
	}
}
...
// empty response — because the auth middleware could not reach the API's writer
// without an import cycle. internal/platform/httperr exists to end that.
//
...
func TestRequireAuthAnswersWithAProblemDocument(t *testing.T) {
	for _, tc := range []struct {
	...
	}{
		{
			...
			handler: requireAuth(&fakeUsers{}, &fakeIdentities{})(nil),
		...
		},
		{
			...
			handler: requireAuth(
				&fakeUsers{getErr: errors.New("connection refused")},
				&fakeIdentities{},
			)(nil),
...
func TestRequireAuthDoesNotEchoTheDatabaseError(t *testing.T) {
	handler := requireAuth(
		&fakeUsers{getErr: errors.New("dial tcp 10.1.2.3:5432: connection refused")},
		&fakeIdentities{},
	)(nil)
