...
func (r *recordingUsers) ByClerkID(context.Context, string) (domain.User, error) {
	return domain.User{}, nil
}

func (r *recordingUsers) Upsert(_ context.Context, clerkUserID string, id domain.Identity) (domain.User, error) {
	r.calls++
	r.clerkID = clerkUserID
	r.identity = id
...
func TestWebhookRejectsAnUnsignedRequest(t *testing.T) {
	...
	r := httptest.NewRequest(http.MethodPost, "/api/webhooks/clerk",
		bytesReader(payload(t, "user_created.json")))
...
func newWebhookHandler(t *testing.T, users application.UserRepository) http.Handler {
	t.Helper()
	h, err := identityhttp.WebhookHandler(testWebhookSecret, application.NewService(users, stubIdentities{}))
	if err != nil {
...
func TestWebhookProvisionsTheUserFromASignedEvent(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	handler.ServeHTTP(rec, signed(t, payload(t, "user_created.json"), time.Now()))
	...
	if users.clerkID != "user_2abcDEF123" {
		t.Errorf("clerk id = %q, want user_2abcDEF123", users.clerkID)
	}
...
func TestWebhookRejectsATamperedBody(t *testing.T) {
	...
	body := payload(t, "user_created.json")
...
func TestWebhookRejectsAStaleDelivery(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	handler.ServeHTTP(rec, signed(t, payload(t, "user_created.json"), time.Now().Add(-10*time.Minute)))
...
func TestWebhookAcceptsTheSameEventTwice(t *testing.T) {
	...
	if users.clerkID != "user_2abcDEF123" {
		t.Errorf("clerk id = %q after the replay", users.clerkID)
	}
}
...
func TestWebhookAsksForARetryWhenTheWriteFails(t *testing.T) {
	users := &recordingUsers{err: errors.New("connection reset by peer")}
	handler := newWebhookHandler(t, users)
...
func TestWebhookHandlerRefusesAnUnusableSigningSecret(t *testing.T) {
	if _, err := identityhttp.WebhookHandler("not-a-svix-secret", application.NewService(&recordingUsers{}, stubIdentities{})); err == nil {
...
func TestWebhookErrorsAreProblemDocuments(t *testing.T) {
	handler, err := identityhttp.WebhookHandler(testWebhookSecret, application.NewService(&recordingUsers{}, stubIdentities{}))
	if err != nil {
...
// stubIdentities satisfies the Clerk-facing port. The webhook never reaches it:
// the payload carries the identity, so there is nothing to go and fetch. A
// panic here would mean the handler took the lazy-provisioning path by mistake.
type stubIdentities struct{}

func (stubIdentities) Fetch(context.Context, string) (domain.Identity, error) {
	panic("the webhook must not call Clerk: the event already carries the identity")
}
