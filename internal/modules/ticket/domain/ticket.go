...
// tickets table into every layer above it, and make adding a column a change to
// the application and the transport at once. The mapping happens once, in the
// repository.
//
// The SLA fields are a cache of what the status history says, never the source
// of truth. docs/spec.md §4.2: the history is the fact, and these are rebuilt
// from it on every write. A consistency test proves they agree.
type Ticket struct {
	ID          uuid.UUID
...
// about a ticket, and that it happens to be stored as a nullable timestamp is
...
// Timeline reduces a history to the running and paused phases an SLA
// calculation works in.
//
