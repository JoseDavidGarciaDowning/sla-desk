...
//
// They moved here from internal/store when the ticket module took ownership of
// tickets and ticket_status_history, and they cover exactly what this module
// owns: the CHECK constraints on those two tables, and the queries that read
// them. Anything whose answer depends on SLA arithmetic lives in internal/app
// instead — a test that spans two modules belongs where the two are connected,
...
//
// Read with raw SQL rather than through the SLA module's repository, and that
// is deliberate. tickets carries a foreign key into sla_policies, so a fixture
// needs a real id; what it does not need is a *code* dependency on the module
// that owns the table. A query in a test fixture cannot be imported, called or
// refactored into one.
func normalPolicyID(t *testing.T, c testContext) int64 {
	t.Helper()
	return policyIDFor(t, c, domain.PriorityNormal)
}
...
func setup(t *testing.T) testContext {
	...
	return testContext{ctx: ctx, tx: tx, q: ticketdb.New(tx)}
}

// newUser writes a users row directly. Same reasoning as normalPolicyID: the
// foreign key needs a real id, and a fixture is not a dependency.
func newUser(t *testing.T, c testContext, clerkID string, role domain.ActorRole) uuid.UUID {
	t.Helper()
	var id uuid.UUID
...
func TestOpenTicketWithoutARunningClockIsRejected(t *testing.T) {
	...
	if name := pgtest.RejectedBy(t, err); name != "tickets_clock_runs_only_while_open" {
...
func TestRunningClockWithoutADueDateIsRejected(t *testing.T) {
	...
	if name := pgtest.RejectedBy(t, err); name != "tickets_due_at_set_iff_clock_running" {
...
func TestNegativeConsumedTimeIsRejected(t *testing.T) {
	...
	if name := pgtest.RejectedBy(t, err); name != "tickets_sla_consumed_not_negative" {
...
func TestDatabaseRejectsAnUnknownCategory(t *testing.T) {
	...
	if name := pgtest.RejectedBy(t, err); name != "tickets_category_valid" {
...
func TestDatabaseRejectsAnUnknownTicketStatus(t *testing.T) {
	...
	if name := pgtest.RejectedBy(t, err); name != "tickets_status_valid" {
...
func TestDatabaseRejectsAnEmptyTitle(t *testing.T) {
	...
	if name := pgtest.RejectedBy(t, err); name != "tickets_title_length" {
...
func TestDeletingAUserWithTicketsIsRejected(t *testing.T) {
	...
	if name := pgtest.RejectedBy(t, err); name != "tickets_requester_id_fkey" {
...
func TestDeletingAPolicyInUseIsRejected(t *testing.T) {
	...
	if name := pgtest.RejectedBy(t, err); name != "tickets_sla_policy_id_fkey" {
...
func TestGetTicketForRequesterHidesAnotherCustomersTicket(t *testing.T) {
	...
	// Sanity: Alice can read her own domain. Without this the test below would
...
func TestSelfTransitionIsRejected(t *testing.T) {
	...
	_, err := c.q.InsertTicketStatusHistory(c.ctx, ticketdb.InsertTicketStatusHistoryParams{
		TicketID:   tk.ID,
		FromStatus: new(domain.StatusOpen),
		ToStatus:   domain.StatusOpen,
		ActorID:    actor,
		ActorRole:  domain.RoleCustomer,
		CreatedAt:  time.Now().UTC(),
	...
	if name := pgtest.RejectedBy(t, err); name != "ticket_status_history_is_a_real_transition" {
...
func TestHistoryComesBackInTransitionOrder(t *testing.T) {
	...
	sequence := []struct {
		...
		{new(domain.StatusOpen), domain.StatusPending},
		{new(domain.StatusPending), domain.StatusOpen},
		{new(domain.StatusOpen), domain.StatusResolved},
	}
...
func TestHistoryIsOrderedByEventTimeNotByID(t *testing.T) {
	...
	second, err := c.q.InsertTicketStatusHistory(c.ctx, ticketdb.InsertTicketStatusHistoryParams{
		TicketID: tk.ID, FromStatus: new(domain.StatusOpen), ToStatus: domain.StatusPending,
		ActorID: actor, ActorRole: domain.RoleAgent, CreatedAt: time.Now().UTC(),
	})
	if err != nil {
...
func policyIDFor(t *testing.T, c testContext, priority domain.Priority) int64 {
	...
	var id int64
	if err := c.tx.QueryRow(c.ctx,
		`SELECT id FROM sla_policies WHERE priority = $1 AND active`, priority).Scan(&id); err != nil {
...
func TestListTicketsByRequesterFiltersByStatusAndPriority(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			name:   "status alone",
			status: new(string(domain.StatusOpen)),
			want:   []string{"open normal", "open urgent"},
		},
		{
			name:     "priority alone",
			priority: new(string(domain.PriorityUrgent)),
			want:     []string{"open urgent", "pending urgent"},
		},
		{
			name:     "both, which intersect",
			status:   new(string(domain.StatusPending)),
			priority: new(string(domain.PriorityUrgent)),
...
func TestFilteringNeverReachesAnotherCustomersTickets(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "no filter"},
		{name: "status only", status: new(string(domain.StatusOpen))},
		{name: "priority only", priority: new(string(domain.PriorityUrgent))},
		{
			name:     "both",
			status:   new(string(domain.StatusOpen)),
			priority: new(string(domain.PriorityUrgent)),
...
// so an empty history means one of those two and never a real domain.
func TestHistoryForRequesterReturnsTheOwnersRowsInOrder(t *testing.T) {
	...
	writeHistory(t, c, tk.ID, alice, new(domain.StatusOpen), domain.StatusPending)
	writeHistory(t, c, tk.ID, alice, new(domain.StatusPending), domain.StatusResolved)
