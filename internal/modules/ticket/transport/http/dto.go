// Package http is the ticket module's HTTP surface: the request and response
// shapes, the validation the API performs, and the handlers that orchestrate
// the use cases.
//
// It decides status codes and serialisation and nothing else. Business rules
// live in the domain and the application layer and are never decided here.
package http
...
//
// Not domain.Ticket: that would put every future field on the wire, and make an
// internal change a breaking API change.
...
//
// Not domain.HistoryEntry. That carries ActorID — another user's
...
func NewTicketHistoryResponse(entries []domain.HistoryEntry) TicketHistoryResponse {
	// make, so an empty timeline encodes as [] rather than null — though the
	// handler answers 404 before it can be empty.
	out := make([]TicketHistoryEntry, 0, len(entries))
	for _, e := range entries {
		out = append(out, TicketHistoryEntry{
	...
	return TicketHistoryResponse{Entries: out}
}
