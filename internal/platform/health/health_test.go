...
// Calls the handler directly rather than through a router, which is what this
// did while it lived in internal/api. What it asserts is how probe results turn
// into a status and a body; reaching that through a router meant an unrelated
// route could redden it. That the endpoint is mounted at Path, and mounted
// without authentication, is asserted where the mounting happens — see
// internal/app/router_test.go.
func healthRequest(t *testing.T, probes map[string]Probe) *httptest.ResponseRecorder {
	t.Helper()

	rec := httptest.NewRecorder()
	Handler(probes).ServeHTTP(rec, httptest.NewRequest(http.MethodGet, Path, nil))
